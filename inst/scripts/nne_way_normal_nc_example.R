...
#
# Kernel: `NneWayNormalNC` (non-centered)
# State (DIM = 2 + J): (mu, log(tau), z_1..z_J) with theta_j = mu + tau*z_j.
#
# Data layout:
...
# Initialisation near empirical summaries (z approx (ybar-mu)/tau).
mu0 <- mean(ybar)
tau0 <- sd(ybar)
...
mu <- theta[, 1]
tau <- exp(theta[, 2])
z <- theta[, 3:ncol(theta), drop = FALSE]
theta_j <- sweep(z, 1, tau, `*`) + mu
...
cat(sprintf("NneWayNormalNC (%s): J=%d, K=%d, it=%d, covit=%d\n", toupper(sampler), J, K, it, covit))
...
mtext(sprintf("NneWayNormalNC: population mean traces (%s), J=%d, K=%d; dashed = true value",
              toupper(sampler), J, K),
      outer = TRUE, cex = 1.05)
par(op)
